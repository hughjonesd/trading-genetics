...
huxreg(mod_bo_psea_base, 
        coefs = c(
         ...
         tidy_args = list(conf.int = FALSE, cluster = list(mf_pairs_reg$couple_id, mf_pairs_reg$n_sibs.x))
       ) %>% 
...
mod_bo_males <- lapply(fml_bo_psea[c(2, 5)], fixest::feols, 
                 data = mf_pairs_reg %>% filter(x == "Male"),
                 notes = FALSE
               )
mod_bo_females <- lapply(fml_bo_psea[c(2, 5)], fixest::feols, 
