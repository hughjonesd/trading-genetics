...
abstract: |
  ...
  economic institutions. \par\textbf{Keywords:} Assortative mating, 
  MoBa, UK Biobank.
date: "`r Sys.Date()`"
output: 
...
# load moba results
load("moba-results/tradinggenetics_moba.Rdata")
```
...
A rich literature in genetics studies assortative mating [@rao1976resolution;
@heath1985resolving; @otto1995genetic]. The most common model is phenotypic
...
[@eaves1999comparing]. Since the "genomics revolution", which has made
...
that GAM takes place in contemporary human populations [@hugh2016assortative;
@robinson2017genetic; @howe2019genetic]. Other work has analysed cross-trait
assortative mating, when e.g. people with genes for height marry people with
genes for intelligence [@Beauchamp_2010; @Sundet_2005;
@border2022cross]. As a result, the two types of genetic variation become
...
[@magnus2016cohort; @paltiel2014biobank]. The Norwegian Mother, Father and Child
...
Regional Committees for Medical and Health Research Ethics XXX.[^boilerplate]

[^boilerplate]: Parts of the above paragraph come from a standard template 
...
polygenic score is equal to the mean of their parents' polygenic scores.[^endog] We
...
# Notes. 
# - Regressions use se = hetero, ask him to do them by pair (or did he?)
# - Ask to preremove those with missing height, BMI, uni, income so
#   that N is standardized
# - What's a good equivalent to self-reported health?
# - Can we get income in the same units (or similar) to the UK?
#   (UK it was £1000, rescaled, from first job pay?) Perhaps doesn't matter,
#   but get real income not deciles if poss... can we get earlier than 30?
#   need to ask about this.
# - Why no income in the subsets regressions?
# - Details of how PSEA was created. In partic, what's its R2 on university
#   attendance in this data? (Want to see if coef of PSEA is comparable; or
#   indeed all coefficients, as spouse PSEA is the DV!) Also, did they
#   exclude Norway from the estimation of the summary stats?
#   - In the end we might want Abdel to replicate Norway v. closely...
# - Can we get a subset of actual siblings? Would be so coool....
# - (Data for a) pic along the lines of Fig 4. Could merge with
#   diff colours?
# - research ethics approval number for boilerplate
# - are there weights?
# - Balance tests on PGS?
...
mediator_effect_bo <- purrr::map_dbl(export_table1_tidyse[-1], 
                                     function (x) {
                                       x$estimate[x$term == "parity"]
                                     })

mediators <- c("university", "income", "height", "bmi")
...
project is publicly available at https://github.com/hughjonesd/trading-genetics,
...
mod_bo_1st_moba <- purrr::map2(export_table1_tidyse[-1],
                                     export_table1_glance[-1], 
                                     convert_moba_for_huxreg)
...
ht <- huxtable::huxreg(reg_list, 
       ...
       set_caption("Regressions of spouse PSEA, separate birth order dummies: subsets (Great Britain)") %>% 
       set_tb_padding(3) %>% 
       set_lr_padding(2) %>% 
       set_font_size(9) %>% 
       set_width(0.95)
