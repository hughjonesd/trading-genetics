...
abstract: |
  ...
  high- and low-income people, including differences related to human capital and
  health). We model social-genetic assortative mating 
...
correlated with cross-sectional inequality [@becker2018theory;@krueger2012rise], 
...
tax rates, the genes-SES gradient is in general weaker.[^not-always]

[^not-always]: "In general" because the relationship can be reversed when
...
studies [@lee2018gene] and is a cause of educational attainment itself, as well
as of intelligence and labour market outcomes. It is already known that humans
mate assortatively on PSEA [@hugh2016assortative;@robinson2017genetic], which
...
wealth. @clark2021bell argues that the data can be explained by an underlying
...
on descendant wealth, genetics is a *confound*; under SGAM, it can be a
*mediator*.
...
@chiappori2018marriage]. We know that there is a "genes-SES gradient": people
...
[@Tambs_1989]. Genome-wide Complex Trait Analysis shows that the family
socio-economic status of 2-year-old children can be predicted from their genes
[@Trzaskowski_2014]. Children born into higher-income families have more genetic
variants predicting educational attainment [@belsky2018genetic]. Studies
...
[e.g. @kaplan2013s;@eika2019educational]. Another important aspect of the
...
[@abdellaoui2019genetic]. There is a correlation between education and health,
which may be mediated by shared genetic causes
[@amin2015schooling;@boardman2015can]. Family SES is a confound for several
health-related polygenic scores [@selzam2019comparing]; SES correlates with
genetic variants for mental psychiatric disorders and substance use
[@marees2021genetic]. Understanding the aetiology of health and disease requires
...
genetics in the long run.[^prop] This poses the problem raised first by
...
`r pretty(eff_own_psea_within * 100)` percentage points ($p < 2 \times 10^{-16}$).
...
directly. Instead, we perform an @oster2019unobservable -style analysis, by asking
...
[@branigan2013variation]. This is the variance of educational attainment 
...
`r percent(R2_ea_psea, 0.01)`. Assuming the classic errors-in-variables model,
the ratio of these two $R^2$ statistics gives the proportion of measured PSEA that is systematic, with the rest being noise. This ratio is then the maximum $R^2$ for any
regression with measured PSEA as the dependent variable: 
`r percent(prop_systematic_psea, 0.01)`. We can reduce this number further by
considering the maximum possible variance explained of spouse's *true* PSEA by
any environmental variable. (Own genetic variables are excluded because, as
discussed, these cannot mediate the effect of birth order.) We take this as 
`r percent(true_psea_R2_max)`. Phenotypic correlations between spouses
are typically much less than $\sqrt{0.5}$, and those for non-SES-related 
phenotypes are smaller still [@clark2021bell], so this seems like a
generous upper bound.[^psea-r2] Multiplying these together gives a maximum $R^2$ of 
`r percent(R2max, 0.01)`. Following @oster2019unobservable, we use this maximum
to estimate the degree $\delta$ of selection on unobservables that would reduce
the effect of university attendance and income in first job to zero in Table
\@ref(tab:tbl-bo-psea). $\delta = 1$ would mean equal selection on observables and
unobservables. Results are $\delta = `r delta_uni`$ for university
attendance, $\delta = `r delta_fjp`$ for income in first job. That is, selection
on unobserved, non-SES-related variables would have to be almost equal to
selection on observed variables (height, IQ, BMI, health) to reduce our effects
to zero.

[^psea-r2]: Measured phenotypes might contain a lot of noise compared to the
"true" phenotypes on which people match, and if so, spousal correlations on 
genotypes might be higher than these numbers. However, @robinson2017genetic estimate 
the $R^2$ of own true PSEA on spouse PSEA as $0.654^2 = 0.43$, so even in this
case an $R^2$ of 50% seems quite high.
