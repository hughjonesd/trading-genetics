...
@ref(tab:tbl-sur) shows the results: we don't include year of birth dummies
...
huxreg(
        "Males: PSEA"          = mod_sur_psea$males, 
        "Males: birth order"   = mod_sur_bo$males, 
        "Females: PSEA"        = mod_sur_psea$females, 
        "Females: birth order" = mod_sur_bo$females,
        coefs = c("Birth order" = "birth_order.x", "PSEA" = "EA3.x"),
        statistics = c(N = "nobs"),
        stars = my_stars
      ) %>% 
        insert_row("Controls (mother's age)", rep("Yes", 4), after = 5) %>% 
        insert_row("Family size dummies", rep("Yes", 4), after = 6) %>% 
        insert_row("Birth month dummies", rep("Yes", 4), after = 7) %>% 
        insert_row("Birth year dummies", rep("No", 4), after = 7) %>% 
        set_bottom_border(6:8, everywhere, 0) %>% 
        set_number_format(10, everywhere, 0) %>% 
        merge_cells(10, 2:3) %>% 
        merge_cells(10, 4:5) %>% 
        set_caption(
          "Seemingly Unrelated Regressions on spouse characteristics")

boot_ratios <- replicate(199, {
  mf_pairs_boot <- mf_pairs_sf %>% slice_sample(prop = 1, replace = TRUE)
  mod_sur_females_boot <- systemfit::systemfit(list_fml, 
                                          data = mf_pairs_boot %>% 
                                                   filter(female.x),
                                          method = "SUR"
                                        )

  mod_sur_males_boot <- systemfit::systemfit(list_fml, 
                                          data = mf_pairs_boot %>% 
                                                   filter(! female.x),
                                          method = "SUR"
                                        )
  ...
  c(
    females_PSEA = coef(mod_sur_females_boot)["PSEA_birth_order.x"]/
                     coef(mod_sur_females_boot)["PSEA_EA3.x"],
    females_BO   = coef(mod_sur_females_boot)["BO_birth_order.x"]/
                     coef(mod_sur_females_boot)["BO_EA3.x"],
    males_PSEA   = coef(mod_sur_males_boot)["PSEA_birth_order.x"]/
                     coef(mod_sur_males_boot)["PSEA_EA3.x"],
    males_BO     = coef(mod_sur_males_boot)["BO_birth_order.x"]/
                     coef(mod_sur_males_boot)["BO_EA3.x"]
  )
})

boot_ratios <- as.data.frame(t(boot_ratios))
names(boot_ratios) <- c("females_PSEA", "females_BO", "males_PSEA", 
                          "males_BO")
boot_ratios$females_diff <- boot_ratios$females_PSEA - boot_ratios$females_BO
boot_ratios$males_diff <- boot_ratios$males_PSEA - boot_ratios$males_BO

boot_ratio_ci_females <- quantile(boot_ratios$females_diff, c(0.025, 0.975))
boot_ratio_ci_males   <- quantile(boot_ratios$males_diff, c(0.025, 0.975))

stopifnot(between(0, boot_ratio_ci_females[1], boot_ratio_ci_females[2]))
stopifnot(between(0, boot_ratio_ci_males[1], boot_ratio_ci_males[2]))
