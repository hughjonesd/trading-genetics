...
ta_early <-  list(
               conf.int = FALSE, 
               cluster = list(
                 mf_pairs_reg$couple_id[mf_pairs_reg$YOB.x <= split_year])
             )
ta_late <-  list(
                conf.int = FALSE, 
                cluster = list(
...
huxreg(reg_list_el,
         ...
         stars = my_stars,
         tidy_args = list(ta_early, ta_early, ta_late, ta_late)
       ) %>% 
...
stopifnot(pval_own_psea_3 > 0.01)
```
...
own PSEA (effect size `r pretty(coef_own_psea_3, 3)`, $p < 0.05$) .
...
huxtable::huxreg(mod_bo_psea_ix, 
       ...
       insert_row(after = 24, "Controls (IQ, height)", 
                    c("No", rep("Yes", 4))) %>% 
       set_bottom_border(22:24, everywhere, 0) %>% 
...
huxreg(mod_bo_psea_no3, 
        coefs = c(
         ...
         tidy_args = list(
           conf.int = FALSE, 
           cluster = list(
             mf_pairs_reg$couple_id[mf_pairs_reg$n_sibs.x != 3],
             mf_pairs_reg$n_sibs.x[mf_pairs_reg$n_sibs.x != 3]
           )
         )
       ) %>% 
       insert_row(after = 15, "Family size dummies", rep("Yes", 5)) %>% 
