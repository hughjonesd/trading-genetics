...
```{r pic-extreme-socs, fig.subcap = c("Caste society", "Egalitarian society"), fig.cap = "Genetic lock-in: example societies"}
sigma <- 0.4
gamma <- 0.5

plot.new()
plot.window(0:1, 0:1, xaxs = "i", yaxs = "i")
box()
adj <- 0.02
rect(0 + adj, 1 - sigma + adj, 1 - adj, 1 - adj, col = "grey90")
text(0 + adj * 2, 1 - sigma + adj *2, "H")
rect(0 + adj, 0 + adj, 1 - adj, 1 - sigma - adj, col = "grey90")
text(0 + adj * 2, 0 + adj *2, "L")
# abline(h = 1 - sigma, lty = 2)
abline(v = 1 - gamma, lty = 2)
mtext(paste("G:", 0:1), side = 1, line = 1, at = c((1 - gamma)/2, (1 - gamma)/2 + 1/2))
mtext(paste("S:", 0:1), side = 2, line = 1, at = c((1 - sigma)/2, (1 - sigma)/2 + 1/2))

len <- 0.1
arrows(
        x0     = 1 - gamma - len,
        x1     = 1 - gamma + len, 
        y0     = c((1 - sigma)/2, 1/2 + (1 - sigma)/2),
        code   = 3, 
        length = 0.1,
        lwd = 1.5
      )
text(0 + 2 * adj, 1 - 2 * adj, paste0("No intermarriage across status.\n",
      "Mix of G remains same over\n", "time in each group."), cex = 0.8, adj = c(0, 1))
...
box()
adj <- 0.02
rect(1 - gamma + adj, 0 + adj, 1 - adj, 1 - adj, col = "grey90")
text(1 - gamma + adj * 2, 0 + adj *2, "H")
rect(0 + adj, 0 + adj, 1 - gamma - adj, 1 - adj, col = "grey90")
text(0 + adj * 2, 0 + adj *2, "L")
abline(h = 1 - sigma, lty = 2)
# abline(v = 1 - gamma, lty = 2)
mtext(paste("G:", 0:1), side = 1, line = 1, at = c((1 - gamma)/2, (1 - gamma)/2 + 1/2))
...
text(0 + 2 * adj, 1 - 2 * adj, paste0("No intermarriage between\n", "G = 0 and 1.\n",
      "Mix of S remains same over\n", "time in each group."), cex = 0.8, adj = c(0, 1))
```
...
fml_bo_psea[[2]] <- EA3.y ~ university.x + EA3.x | factor(n_sibs.x)
fml_bo_psea[[3]] <- EA3.y ~ university.x + n_older_sibs.x + EA3.x | factor(n_sibs.x)
fml_bo_psea[[4]] <- EA3.y ~ income_cat.x + n_older_sibs.x + EA3.x | factor(n_sibs.x)
fml_bo_psea[[5]] <- EA3.y ~ income_cat.x + n_older_sibs.x + EA3.x | factor(n_sibs.x)

mf_pairs_reg <- mf_pairs_twice %>% 
                  filter(n_sibs.x >= 2, n_sibs.x <= 8, ! is.na(n_older_sibs.x))
mod_bo_psea <- lapply(fml_bo_psea, fixest::feols, 
                 data = mf_pairs_reg,
                 notes = FALSE
               )

# vcov_bo_psea <- lapply(mod_bo_psea, vcov, cluster = list(mf_pairs_reg$couple_id))
...
huxreg(mod_bo_psea_robust, 
       ...
       insert_row(after = 9, "Family size dummies", rep("Yes", 5)) %>% 
       insert_row(after = 10, "Polygenic score controls", rep("Yes", 5)) %>% 
