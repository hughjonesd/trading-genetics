...
#TODO
# Here:
# Finally, our paper challenges the widespread assumption, in economics and beyond,
#  that genetic endowment is ex- ogenous to economic characteristics. 
# - Need to put some citations in. Maybe Plomin. See also below, though, for
...
Bit for conclusion:
  ...
  More  generally, SGAM is likely to apply to a historically wide range of societies. Whilst a degree of meritocracy exists in modern capitalist economies, opportunities to earn status have been far more limited in most societies throughout history (Smelser and Lipset 1966). On the other hand, assortative mating is likely to be a cultural universal. Thus, SGAM predicts that genetic differences across social status are likely to be historically ancient.
...
attractiveness in marriage markets.[^status-SES] As a result, genetics and
...
socio-economic status [@belsky2018genetic; @Rimfeld_2018;
...
derives from large-scale genome-wide association studies [@lee2018gene] and
...
phenotypes [@hugh2016assortative]. We depart from the assumption that both
...
off physical characteristics for higher earnings [e.g. @chiappori2012fatter] or matching social class [@banerjee2013marry].
...
similarity [see e.g. @robinson2017genetic]. In order to isolate the causal link
...
@heckman2013understanding (henceforth HPS), who decompose the average treatment
...
have not formalised the mechanism and consequences of SGAM.[^genetics-papers]

[^genetics-papers]: @halsey1958genetics showed in a two-class model that social
...
huxreg(mod_bo_1st, 
       ...
       set_width(0.8) %>% 
       set_caption("Regressions of socio-economic status variables on birth order")
...
compute_mediation_effects <- function (resample = FALSE) {
  ...
  mod_bo_psea_boot <- purrr::map(fml_bo_psea, fixest::feols, 
                                  data = mf_pairs_tmp, notes = FALSE)
  mod_bo_overall_boot <- purrr::map(mod_bo_psea_boot[-1], ~{
                                    removed <- .x$obsRemoved
                                    # ensure we always use the same data
                                    # for esetimating the "overall" coef.
                                    data <- mf_pairs_tmp[-removed,]
                                    fixest::feols(fml_bo_psea[[1]], 
                                                    data = data, 
                                                    notes = FALSE)
                                  })
  ...
  psea_effect_bo <- purrr::map_dbl(mod_bo_overall_boot, 
                                     list(coef, "birth_order.x"))
...
n_reps <- 199
bootstraps <- replicate(n_reps, compute_mediation_effects(resample = TRUE))
...
without mediators. 90% confidence intervals are estimated by `r n_reps`
bootstraps.
...
variance.[^10] We regress each score on own birth order, controlling for
...
*optimates* ("best").[^11] This belief has been explained by the
...
fml_sur_psea <- EA3.y ~ birth_order.x + EA3.x + moth_age_birth.x + 
...
suppressWarnings(huxreg(
        "PSEA"          = mod_sur_psea$males, 
        "Birth order"   = mod_sur_bo$males, 
        "PSEA"        = mod_sur_psea$females, 
        "Birth order" = mod_sur_bo$females,
        coefs                  = c(
                                    "Birth order" = "birth_order.x", 
                                    "PSEA"        = "EA3.x"
                                 ),
        ...
        insert_row("Controls (mother's age)", rep("Yes", 4), after = 6) %>% 
        ...
        # merge_cells(10, 2:3) %>% 
        # merge_cells(10, 4:5) %>% 
        # set_align(10, everywhere, "centre") %>% 
        set_caption(
