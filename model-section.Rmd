...
```{r pic-asymptotic-ineq, fig.align = "center", fig.cap = "Long-run inequality in SES (standard deviation of $x_2$) by weight of genetics in spouse matching ($a$) and strength of inheritance of SES ($\\theta$). $\\tau$ = 0.95.", dpi = 300}
...
rgl::mtext3d("Std. dev. x₂", "z-+", line = 6)
```


Figures \@ref(fig:pic-asymptotic-corr) and \@ref(fig:pic-asymptotic-ineq) plot the asymptotic correlation between $x_1$ and $x_2$, and the
asymptotic standard deviation of $x_2$, for $\tau = 0.95$. The correlation 
between $x_1$ and $x_2$ is maximized for intermediate levels of $a$, but economic
inequality is highest when assortative mating is purely on SES and $a = 0$.
