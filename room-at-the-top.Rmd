...
```{r pic-extreme-socs, fig.subcap = c("Caste society", "Egalitarian society"), fig.cap = "Genetic lock-in: example societies", fig.width = 2.5}
sigma <- 0.4
gamma <- 0.5
...
```{r pic-theory, fig.cap = "Social stratification and genetic lock-in: theory", fig.height = 3.5}
...
bo_prop_est <- c(
  coef(mod_bo_psea[[2]])["n_older_sibs.x"]/orig_coef,
  coef(mod_bo_psea[[3]])["n_older_sibs.x"]/orig_coef,
  coef(mod_bo_psea[[4]])["n_older_sibs.x"]/orig_coef
)
...
Column 2 includes university attendance. Column 3
includes income. Column 4 includes both. We calculate the percentage decrease in
the effect of birth order across the columns, and estimate 95% confidence intervals
for this figure by running bootstraps (N = `r n_boot`). Including university
attendance alone reduces the effect of birth order by  `r bo_prop_est[1]`
