...
```{r pic-bo-psea-interaction, fig.align = "center", fig.cap = "Regressions of spouse PSEA: birth order dummies within different family sizes. Labels show birth order. Lines are 95% confidence intervals clustered by couple ID and family size."}
...
mod_bo_psea_full <- fixest::feols(fml_bo_psea_full, mf_pairs_reg, 
                                    notes = FALSE)
coefs <- tidy(mod_bo_psea_full, conf.int = TRUE, 
            cluster = list(mf_pairs_reg$couple_id, mf_pairs_reg$n_sibs.x))
...
ggplot(coefs, aes(estimate, `Family size`, colour = `Birth order`)) + 
