...
huxtable::huxreg(mod_bo_psea_dummies, 
       ...
       insert_row(after = 24, "Family size dummies", rep("Yes", 5)) %>% 
       insert_row(after = 25, "Birth month dummies", rep("Yes", 5)) %>% 
       insert_row(after = 26, "Birth year dummies", rep("Yes", 5)) %>% 
...
mod_bo_males_dummies <- lapply(fml_bo_psea_dummies[c(2, 3)], 
                                 fixest::feols, 
                                 data = mf_pairs_reg %>% 
                                   filter(x == "Male"),
                                 notes = FALSE
                        )
mod_bo_females_dummies <- lapply(fml_bo_psea_dummies[c(2, 3)], 
                                  fixest::feols, 
                                  data = mf_pairs_reg %>% 
                                    filter(x == "Female"),
                                  notes = FALSE
...
mf_pairs_children <- mf_pairs_reg %>% 
                       filter(n_children.x > 0, n_children.y > 0)
mod_bo_children_dummies <- lapply(fml_bo_psea_dummies[c(2, 3)], 
                             fixest::feols, 
                             data = mf_pairs_children,
                             notes = FALSE
                           )
