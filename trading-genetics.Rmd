...
`r R2max_moba`. For university attendance, $\delta^*$ is `r delta_uni_moba`, 
...
glance_uni_psea_moba <- data.frame(
  adj.r.squared = 0.182223,
  nobs          = 9729L
)

tidy_inc_psea_moba <- data.frame(
  term      = c("eapgsresid"),
  estimate  = c(-0.000734),
...
glance_inc_psea_moba <- data.frame(
...
suppressWarnings(
  huxreg(list("University" = tidy_uni_psea_moba, "Income" = tidy_inc_psea_moba),
