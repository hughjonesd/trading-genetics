...
[^not-direct]: Note that we do not assume that people match *directly* on 
...
```{r pic-asymptotic-corr, fig.align = "center", fig.cap = "Long-run correlation between genetics $x_1$ and SES $x_2$, by weight of genetics in spouse matching ($a$) and SES inheritance ($\\theta$). $\\tau$ = 0.95.", dpi = 300, out.height="3.5in"}
...
```{r pic-gamma, fig.align = "center", fig.cap = "Correlation between children's genetics $x^\\prime_1$ and children's SES $x^\\prime_2$, by weight of genetics in spouse matching ($a$) and SES inheritance ($\\theta$), for meritocracy $\\gamma$ = 0.15. Other parameters: $\\tau$ = 0.95, $s$ = $S$ = 1, $\\sigma$ = 0.", dpi = 300, out.height = "4in"}

# "if we compute the asymptotic correlation": see new-model-computation.R
# for code
...
rgl::plot3d(\(theta, a) corr_x1_x2(s = 1, S = 1, sigma = 0, a = a, gamma = 0.15,
                                   tau = 0.95, theta = theta), 
            zlab  = "", 
            xlab  = "θ", 
            col   = "darkred", 
            front = "lines",
...
@harden2021genetic calls the "genetic lottery".
