...
[@clark2015intergenerational; @solon2018we].
...
intergenerational absolute mobility has declined [@western2008inequality; @chetty2017fading].[^stats] Assortative mating in marriage markets can increase the variance of human 
capital and income across families [@breen2011educational; @greenwood2014marry],
and increasing returns to human capital may explain rising inequality [e.g. @kaplan2013s; @becker2018theory]. It
...
only recently [@hugh2016assortative; @robinson2017genetic]. Similarly, wealthy 
families pass on advantages to their children through both genetic inheritance 
and environmental influence [@Rimfeld_2018; @bjorklund2006origins].[^review]
...
matching social status [@banerjee2013marry].
...
2 is a measure of social status, such as income. The traits contribute to each individual's attractiveness, which for men is given by

\[
i(x) = ax_1 + (1-a)x_2.
\]

Here $a$ measures the relative importance of genetics against social status
in the marriage market. For now we assume that women's attractiveness is the same:
...
In particular, the distribution of total household income has variance

\[
V\left( x_{2}+y_{2}\right) =2S^{2}+2C^{2}>2S^{2} 
\]

so that assortative mating increases cross-sectional inequality compared to
random mating. Note, however, that 

\[
V\left( x_{2}+y_{2}\right) \leq 4S^{2} 
\]

which would be reached only if $a = 0$, i.e. people only match on income.

## Children

We next specify how children inherit traits $x_1$ and $x_2$ from their parents.
For child traits $x'$, with parental traits $x$ and $y$, we assume:

\begin{eqnarray}
x_{1}^{\prime } &=&\frac{x_{1}+y_{1}}{4}+\varepsilon  \\
x_{2}^{\prime } &=&\gamma \left( \frac{x_{1}+y_{1}}{2}+\varepsilon \right) +%
\frac{x_{2}+y_{2}}{4}+\eta   \nonumber
\end{eqnarray}

where $\varepsilon ~ N(0, 1)$ and $\eta ~ N(0, 1)$ are independent shocks.

XXX we probably want $x'_1 = (1-\varepsilon)(x_1+y_1)/2$ again.

Thus, genetics are inherited from parents equally, but with some regression to 
the mean imposed by mutation and/or selection.... Children's income is allowed to
depend on both their own genetics, and on their parents' income. The parameter
$\gamma$ represents "meritocracy", in the sense that it lets talented individuals
increase their income.

## Benchmark: random matching
...
\frac{x_{2}+y_{2}}{4}+\eta   \nonumber
\end{eqnarray}%
gives the covariance matrix for children:%
\begin{eqnarray*}
\mathbb{C} &=&\left( 
\begin{array}{cc}
1 & \gamma  \\ 
...
\frac{\gamma }{2} & \frac{1}{4} & \frac{\gamma }{2} & \frac{1}{4}%
\end{array}%
...
&=&\left( 
\begin{array}{cc}
\frac{1}{8}s^{2}+1 & \frac{1}{4}\gamma s^{2}+\frac{1}{8}\sigma +\gamma  \\ 
\frac{1}{4}\gamma s^{2}+\frac{1}{8}\sigma +\gamma  & \frac{1}{8}S^{2}+\frac{1%
}{2}s^{2}\gamma ^{2}+\gamma ^{2}+\frac{1}{2}\sigma \gamma +1%
\end{array}%
\right) \allowbreak 
\end{eqnarray*}
...
x_{2}^{\prime } &=&\gamma \left( \frac{x_{1}+y_{1}}{2}+\varepsilon \right) +%
\frac{x_{2}+y_{2}}{4}+\eta   \nonumber \\
&=&\gamma \frac{x_{1}+y_{1}}{2}+\frac{x_{2}+y_{2}}{4}+\eta +\gamma
\varepsilon 
\end{eqnarray}%
...
\begin{array}{c}
.5\frac{x_{1}+y_{1}}{2} \\ 
\gamma \frac{x_{1}+y_{1}}{2}+.5\frac{x_{2}+y_{2}}{2}%
\end{array}%
\right) =\left( 
\begin{array}{cccc}
\frac{1}{4} & 0 & \frac{1}{4} & 0 \\ 
...
hence the covariance matrix for children:
...
&=&\left( 
\begin{array}{cc}
\frac{1}{8}A^{2}+\frac{1}{8}s^{2}+1 & \frac{1}{4}\gamma A^{2}+\frac{1}{8}CA+%
\frac{1}{4}\gamma s^{2}+\frac{1}{8}\sigma +\gamma \\ 
\frac{1}{4}\gamma A^{2}+\frac{1}{8}CA+\frac{1}{4}\gamma s^{2}+\frac{1}{8}%
\sigma +\gamma & \frac{1}{2}A^{2}\gamma ^{2}+\frac{1}{2}AC\gamma +\frac{1}{8}%
C^{2}+\frac{1}{8}S^{2}+\frac{1}{2}s^{2}\gamma ^{2}+\gamma ^{2}+\frac{1}{2}%
\sigma \gamma +1%
\end{array}%
\right) \allowbreak
\end{eqnarray*}

In particular, the covariance between children's $x'_1$ and $x'_2$ is positive
even when $\gamma = \sigma = 0$. In other words, socio-genetic assortative mating
creates a genes-income gradient, even in non-meritocratic societies.

## Asymptotics

We also examine asymptotic convergence to stable distributions, where
$\mathbb{C}(x_1, x_2) = \mathbb{C}(x'_1, x'_2)$. Under the random matching
...
\begin{eqnarray*}
s^{2} &=&\frac{8}{7} \\
\sigma  &=&\frac{72}{49}\gamma  \\
S^{2} &=&\frac{904}{343}\gamma ^{2}+\frac{8}{7}.
\end{eqnarray*}
