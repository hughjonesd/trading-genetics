...
my_note <- "{stars}. Standard errors: robust. Weights from Alten et al. (2022)."
...
siblings' birth order is independent of their genetic endowments.[^interaction]

[^interaction]: Though @muslimova2020dynamic find that PSEA and birth order do interact to produce human capital.
...
in contemporary human populations [@howe2019genetic; @hugh2016assortative;
@robinson2017genetic]. Geneticists have also developed the concept of
cross-trait assortative mating [@Beauchamp_2010; @Sundet_2005;
...
outcomes [@marees2021genetic]. Understanding the aetiology of health and disease
...
long run.[^prop] This poses the problem raised first by @young1958rise, and more
...
We use weights from @vanalten2022reweighting, which match the Biobank sample to
its sampling frame (40-69 year olds living close to 22 assessment centres).[^weights]

[^weights]: All our results are robust to using unweighted regressions.
...
mod_bo_1st <- purrr::map(fml_bo_1st, fixest::feols, data = mf_pairs_reg, 
                           notes = FALSE, weights = ~ weights.x)

coef_bo <- purrr::map(mod_bo_1st, tidy) %>%
             purrr::map(filter, term == "birth_order.x") %>% 
             purrr::map_dbl("estimate")

tbl_note <- paste("Estimates from weighted OLS regressions with the ",
...
tbl_note <- paste("Estimates from weighted OLS regressions with spouse PSEA as dependent",
...
tbl_note <- paste("Estimates from weighted OLS regressions corresponding to",
...
check_cor <- function (score_name) {
  ...
  mod_check <- lm(fml_check, mf_pairs_reg, weights = weights.x)
  mod_check %>% 
        tidy() %>% 
...
check_cor_within <- function (score_name) {
  ...
  mod_check <- lm(fml_check_within, mf_pairs_reg, weights = weights.x)
...
# neuroticism and sc_substance_use have p value < 0.1/33
...
family size. Two scores were significant at $p$ < 0.10/33, with effect sizes of
...
of the `r n_within_coefs` birth order coefficients was significant at $p$ <
...
We thank @vanalten2022reweighting for the use of their code to create weights.
...
mod_bo_psea_dummies <- purrr::map(fml_bo_psea_dummies, ~lm(.x, 
                         data = mf_pairs_reg, weights = weights.x))
...
huxreg(mod_bo_psea_pgs, 
         ...
         note = paste(my_note,
                 "\nPolygenic scores: neuroticism, substance use."),
         stars = my_stars,
         tidy_args = list(
           conf.int = FALSE, 
