...
i.e. assortative mating by social background. @otto1995genetic allow
...
$x_2$, and reject the null of no effect, we can reject $a = 1$.[^8] In other
...
these in favour of “social” explanations [kristensen2007explaining; @black2011older].
@cochran2013paternal report that mutational load is approximately linear in 
...
later children. @kantarevic2006birth show that mother's age at
...
spouse SES could be *mediated* by own SES, cf. the extended model of meritocracy in
...
# We remove the single 1938-born person since this causes instability in the
# estimates when using YOB dummies
mod_bo_psea_dummies <- purrr::map(fml_bo_psea_dummies, ~lm(.x, 
                         data = mf_pairs_reg %>% filter(YOB.x != 1938)))
...
fml_with_pnr[[1]] <- with_partner ~ birth_order + EA3_excl_23andMe_UK +
  factor(birth_mon) | factor(n_sibs) + factor(age_at_recruitment)
fml_with_pnr[[2]] <- with_partner ~ birth_order + EA3_excl_23andMe_UK +
  factor(birth_mon) + par_age_birth | factor(n_sibs) + factor(age_at_recruitment)

fml_with_pnr <- lapply(fml_with_pnr, Formula::as.Formula)

mod_with_pnr <- lapply(fml_with_pnr, fixest::feols, 
                           data = famhist %>% filter(birth_order <= 6),
                           notes = FALSE)
huxtable::huxreg(mod_with_pnr, 
                 coefs = c("Birth order" = "birth_order", 
                           "PSEA" = "EA3_excl_23andMe_UK", 
                           "Parents' age at birth" = "par_age_birth"), 
                 note = "{stars}. Other controls: dummies for birth month, number of siblings, and own age",
                 stars = my_stars) %>%
  set_width(0.8) %>%
  set_caption("Regressions of being with a partner vs. single (Great Britain)")
