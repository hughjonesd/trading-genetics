...
[@clark2015intergenerational; @solon2018we]. Intergenerational mobility is
correlated with cross-sectional inequality [@becker2018theory;
@krueger2012rise], which has risen dramatically in high-income countries, at the
same time as intergenerational absolute mobility has declined
[@western2008inequality; @chetty2017fading].[^stats] Assortative mating in
marriage markets can increase the inequality of human capital and income
across families [@breen2011educational; @greenwood2014marry]. It follows that 
...
[@hugh2016assortative;@robinson2017genetic]. Similarly, wealthy families pass on 
...
intergenerational transmission of income see @black2010recent.
...
persistence of inequality [@clark2015intergenerational; @solon2018we]. At the
...
genetic variants [@belsky2018genetic; @Rimfeld_2018;@bjorklund2006origins]. The 
...
[@hugh2016assortative; @robinson2017genetic]. 
...
observed measures of) actual educational attainment [@okbay2022polygenic]. To
...
[@black2011older;@booth2009birth;@Lindahl_2008]. At the same time, the facts of
...
@heckman2013understanding, decomposing the treatment effect into effects of
...
typically focused on educational similarities [e.g.
...
[@Beauchamp_2010;@Sundet_2005]. This happens when people with genes for e.g.
...
@clark2015intergenerational show using a database of surnames that long-run
intergenerational persistence of wealth is higher than simple parent-child
correlations would predict. @clark2021bell argues that the data can be explained
...
on child wealth, genetics is a *confound*; under SGAM, it may also be a
*mediator*.

SGAM also affects cross-sectional inequality, like other forms of assortative
mating [@fernandez2001sorting; @fernandez2005love; @eika2019educational; @chiappori2018marriage]. 
...
human capital [e.g. @bjorklund2006origins]. Thus, the genes-SES gradient is an
...
increase in market returns to human capital [e.g. @kaplan2013s;
@eika2019educational].
...
children, leading to an association between the two [@belsky2018genetic]. This
...
post-communist society allowed higher returns to talent [@Rimfeld_2018]. By
...
*intergenerational mobility*, like mass education or inheritance taxation, may 
in the long run affect not only economic but genetic inequality. Conversely, an 
increase in *economic meritocracy* increases the association between SES and
genetics in the long run.[^prop4] This poses the problem raised first by
@young1958rise, and more recently by @markovits2019meritocracy: meritocracy 
may be self-limiting or even self-undermining. 

[^prop4]: See Proposition \@ref(prop-gamma) below.
...
economics or the social sciences that analyses SGAM or its consequences.[^prev-work]

[^prev-work]: @halsey1958genetics showed in a two-class model that social mobility combined
...
with average earnings by SOC from @ONS2007ASHE. Job codes are only available for 
...
"polygenic score for educational attainment, which is normalized with mean 0 and",
"standard deviation 1. We include parents' age at birth (the mean of parents'",
"ages), and further controls in columns 2-3 to ensure the balance of covariates",
...
"and standard deviation 1. We include mean of parents’ ages at birth,",
"potential non-SES mediators (fluid IQ, height, BMI, self-reported health)",
"and further controls to ensure the balance of",
...
"\\ref{tab:tbl-bo-psea}, columns 2 to 4, that are explained by mediators",
"(SES: university attendance, income in first job; non-SES: fluid IQ, height, ",
"BMI, self-reported health).")
...
"mean 0 and standard deviation 1. We include parents’ age at birth (the mean of",
"parent’s ages) and further controls to ensure the balance of", 
"covariates across birth order. All data is from the UK Biobank for a sample of", 
"UK individuals born between 1935 and 1970.", my_note)
...
natural. As @marx1844economic wrote, "History is the true natural history of
...
huxtable::huxreg(mod_bo_psea_dummies, 
       ...
       insert_row(after = 23, "Controls (IQ, height, BMI, s.-r. health)", 
                    c("No", rep("Yes", 3))) %>% 
...
huxtable::huxreg(reg_list, 
       ...
       insert_row(after = 21, "Controls (IQ, height, BMI, s.-r. health)", 
                    rep("Yes", 6)) %>% 
