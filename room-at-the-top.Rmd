...
huxreg(mod_bo_psea_base, 
         ...
         stars = my_stars,
         note = "{stars}. Standard errors clustered by assessment centre. Respondents with 1-7 siblings included.",
         tidy_args = list(
           conf.int = FALSE, 
...
huxreg(mod_bo_psea, 
        coefs = c(
         ...
         note = "{stars}. Standard errors clustered by assessment centre.",
         stars = my_stars,
         tidy_args = list(
           conf.int = FALSE, 
           cluster = list(mf_pairs_reg$f.54.0.0.x)
...
huxreg(reg_list,
         ...
         note = "{stars}. Standard errors clustered by assessment centre.",
         stars = my_stars,
         tidy_args = list(ta_m, ta_m, ta_f, ta_f, ta_ch, ta_ch)
       ) %>% 
...
```{r pic-bo-psea-interactions, fig.align = "center", fig.cap = "Regressions of spouse PSEA: birth order dummies within different family sizes. Labels show birth order. Lines are 95 per cent confidence intervals clustered by assessment centre. The dummy for birth order 8 is not shown."}
...
fml_bo_psea_pgs <- fml_bo_psea %>% 
                        purrr::map(update, 
                           . ~ . + 
                           age_at_menopauze_resid.x + 
                           caffeine_resid.x + 
                           coronary_artery_disease_resid.x +
                           neuroticism_resid.x
                        )
...
huxreg(mod_bo_psea_pgs, 
         ...
         note = "{stars}. Standard errors clustered by assessment centre. 
                 Polygenic scores: age at menopause, caffeine, coronary artery disease, neuroticism.",
         tidy_args = list(
           conf.int = FALSE, 
           cluster = list(mf_pairs_reg$f.54.0.0.x)
         )
