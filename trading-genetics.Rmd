...
abstract: |
  ...
  physical and mental health. We model social-genetic assortative mating 
...
that is systematic, with the rest being noise: `r percent(R2_ea_psea, 0.01)`/40% 
= `r percent(prop_systematic_psea, 0.01)`. This is then the maximum $R^2$ for 
...
*true* PSEA by any environmental variable. (Own genetic variables are excluded
because, as discussed, these cannot mediate the effect of birth order.) We take
this as `r percent(true_psea_R2_max)`. Phenotypic correlations between spouses
are typically much less than $\sqrt{0.5}$, and those for non-SES-related
phenotypes are smaller still [@clark2021bell], so this seems like a generous
upper bound.[^psea-r2] Multiplying these together gives a maximum $R^2$ of 
`r percent(true_psea_R2_max)` $\times$  `r percent(prop_systematic_psea, 0.01)`
= `r percent(R2max, 0.01)`. Following @oster2019unobservable, we use this
...
observables and unobservables. Results are $\delta = `r delta_uni`$ for
university attendance, $\delta = `r delta_fjp`$ for income in first job. That
