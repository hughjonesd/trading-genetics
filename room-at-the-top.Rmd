...
abstract: |
  ...
  exogenous to own genetic endowments (within families). Spouses of 
  earlier-born siblings have higher polygenic scores for educational attainment. 
  The effect of birth order appears to be mediated by own educational attainment. 
  Thus, environmental shocks to social status are reflected in the DNA 
  of subsequent generations. This mechanism provides a new approach to understanding
  the inheritance of inequality, in contemporary societies and across human
  history. 
...
Income inequality persists over generations. Families are part of this
story, because wealth, human capital and other traits are transmitted
from parents to children. In particular, assortative mating (when
similar people marry each other) can increase social inequality in
income, wealth, social status or human capital, compared to the case
where people pair at random. It is often thought that the degree of
assortative mating has increased in recent generations, and that this
could help to explain increasing levels of social inequality
[@fernandez2005love; @schwartz2005trends; @greenwood2014marry;
@eika2019educational].
...
Genome-wide Complex Trait Analysis (GCTA) shows that even 2-year-old
...
leading explanation for this is that in meritocratic societies,
individuals with genes associated with valuable skills and personality
traits experience upward mobility. Their children will then inherit both
their genes (via biology) and their increased social status (via social
institutions).
...
Under this mechanism, shocks to an individual's social status may be
reflected in the genetics of his or her children. Thus, in contrast to a
large body of research that treats genetic data as an independent
variable and traces its effects on social and economic outcomes, we make
the genetics endogenous.[^2] Our mechanism gives a new explanation for
the link between social status and genetics. This explanation is likely
to apply to a wider range of societies than the alternative
"meritocratic mobility" explanation. That is because, while widespread
social mobility is relatively rare and historically recent, assortative
mating is a human universal [@buss1989sex].
...
We then test for SGAM using contemporary data from the UK. To pin down
SGAM, and avoid the potential confound of pure GAM, we use a causal
antecedent of social status which is independent of individual genetics:
birth order. Birth order is known to affect outcomes including income,
occupational status and educational attainment. We show that people born
earlier within a sibling group marry spouses with different genetics,
specifically, higher polygenic scores for educational attainment. We
present evidence that this effect is mediated by measures of SES,
including educational attainment and labour market income.
...
match. And Donald Trump claimed: "part of the beauty of me is that I am
very rich". Several empirical papers examine cross-trait matching in
marriage markets. @chiappori2012fatter show empirically that body mass
index and SES (specifically, earnings and education) are both valued in
a match. @oreffice2010anthropometry show that height and BMI are
associated with spouse earnings. @dupuy2014personality find spouse
matching on multiple independent dimensions, including education,
height, BMI and personality; see also @chiappori2018bidimensional.
...
They also do not theorize the consequences of SGAM. Within genetics, few
papers have analysed the SGAM mechanism. @halsey1958genetics showed in a
two-class model that social mobility combined with assortative mating
...
huxreg(mod_bo_psea_base, 
         ...
         stars = my_stars,
         note = "{stars}. Standard errors clustered by spouse pair and family size. Respondents with 1-7 siblings included.",
         tidy_args = list(
           conf.int = FALSE, 
           cluster  = list(mf_pairs_reg$couple_id, mf_pairs_reg$n_sibs.x)
         )
       ) %>% 
...
huxreg(mod_bo_psea, 
        coefs = c(
         ...
         note = "{stars}. Standard errors clustered by spouse pair and family size.",
         stars = my_stars,
         tidy_args = list(
           conf.int = FALSE, 
           cluster = list(mf_pairs_reg$couple_id, mf_pairs_reg$n_sibs.x)
...
```{r tbl-bo-early-late, eval = FALSE}

mod_bo_pre55 <- lapply(fml_bo_psea[c(2, 3)], fixest::feols, 
                 data = mf_pairs_reg %>% filter(YOB.x <= 1955),
                 notes = FALSE
               )

mod_bo_post55 <- lapply(fml_bo_psea[c(2, 3)], fixest::feols, 
                 data = mf_pairs_reg %>% filter(YOB.x > 1955),
                 notes = FALSE
               )

reg_list_el <- list(
                 "Born 1934-55" = mod_bo_pre55[[1]],
                 "Born 1934-55" = mod_bo_pre55[[2]],
                 "Born 1956-70" = mod_bo_post55[[1]],
                 "Born 1956-70" = mod_bo_post55[[2]]
               )

ta_pre55 <-  list(
               conf.int = FALSE, 
               cluster = list(
                 mf_pairs_reg$couple_id[mf_pairs_reg$YOB.x <= 1955])
             )
ta_post55 <-  list(
                conf.int = FALSE, 
                cluster = list(
...
huxreg(reg_list_el,
         ...
         stars = my_stars,
         tidy_args = list(ta_pre55, ta_pre55, ta_post55, ta_post55)
       ) %>% 
...
family size dummies in the regression.[^10]
...
polygenic scores.[^11] We regress each score on birth order, controlling
for family size. No scores were significant at $p < 0.10/33$. `r n_10`
...
*kaloi kagathoi* ("fine and good"), while the Roman nobility were the
*optimates* ("best").[^12] This belief has been explained by the human
tendency to believe in a just world [@furnham1993just], or as an
ideology promoted by the dominant class [e.g. @gramsci1971selections].
...
natural history of man".[^13] Genetic endowments can even be viewed as
...
[^13]: He also wrote (ibid.) "I am ugly, but I can buy the most
...
huxtable::huxreg(mod_bo_psea_ix, 
                   ...
                   tidy_args = list(
                     conf.int = FALSE, 
                     cluster = list(
                       mf_pairs_reg$couple_id, 
                       mf_pairs_reg$n_sibs.x
                     )
                   )
                 ) %>% 
...
huxreg(mod_bo_psea_fake, 
         ...
         stars = my_stars,
         note = "{stars}. Standard errors clustered by pair and family size. Respondents with 1-7 siblings included.",
         tidy_args = list(
           conf.int = FALSE, 
           cluster  = list(fake_pairs_reg$couple_id, fake_pairs_reg$n_sibs.x)
         )
       ) %>% 
       insert_row(after = 7, "Family size dummies", rep("Yes", 3)) %>% 
       insert_row(after = 8, "Birth month dummies", "No", "Yes", "Yes") %>% 
