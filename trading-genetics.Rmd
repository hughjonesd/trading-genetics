...
res_uni <- robomit::o_delta(y = "EA3.y", x = "university.x", 
                            con = "first_job_pay.x + fluid_iq.x + height.x + 
                                   bmi.x + sr_health.x", 
...
res_fjp <- robomit::o_delta(y = "EA3.y", x = "first_job_pay.x", 
                            con = "university.x + fluid_iq.x + height.x + bmi.x +
                                   sr_health.x", 
                            m = "EA3.x + par_age_birth.x + factor(n_sibs.x) +
                                 factor(YOB.x) + factor(birth_mon.x)", 
                            R2max = R2max, type = "lm", data = mf_pairs_reg)
...
the ratio of these two $R^2$ statistics, gives the proportion of measured PSEA that is systematic, with the rest being noise. This is then the maximum $R^2$ for any
...
are typically much less than $\sqrt{0.5}$, and non-SES-related 
phenotypes are smaller still [@clark2021bell], so this seems like a
generous upper bound. Multiplying these together gives a maximum $R^2$ of 
...
\@ref(tab:tbl-bo-psea). Results are $\delta = `r delta_uni`$ for university
...
ht <- huxtable::huxreg(mod_bo_psea_dummies, 
                   ...
                   note      = paste(my_note, 
                     "\nGrey background: coefficients are larger than column 1."),
                   stars     = my_stars,
                   tidy_args = tidy_args
                 ) %>% 
...
ht <- huxtable::huxreg(reg_list, 
                   ...
                   note = paste(my_note, "\nGrey background: birth dummy",
                                "coefficient is larger than corresponding",
