...
ta <-  list(conf.int = FALSE, cluster = list(mf_pairs_children$f.54.0.0.x))
...
huxreg(mod_bo_no_moth_age, 
        coefs = c(
         ...
         note = "{stars}. Standard errors clustered by spouse pair.",
         tidy_args = list(conf.int = FALSE, cluster = list(mf_pairs_reg_big$f.54.0.0.x))
       ) %>% 
...
huxreg(mod_bo_psea_no3, 
        coefs = c(
         ...
         tidy_args = list(
           conf.int = FALSE, 
           cluster = list(
             mf_pairs_reg$f.54.0.0.x[mf_pairs_reg$n_sibs.x != 3]
           )
         )
       ) %>% 
       insert_row(after = 15, "Family size dummies", rep("Yes", 5)) %>% 
...
huxreg(mod_bo_psea_fake, 
         ...
         tidy_args = list(
           conf.int = FALSE, 
           cluster  = list(fake_pairs_reg$f.54.0.0.x)
         )
       ) %>% 
       insert_row(after = 7, "Family size dummies", rep("Yes", 3)) %>% 
       insert_row(after = 8, "Birth month dummies", "No", "Yes", "Yes") %>% 
