...
mediation_pct <- function (mediator, mod_1st, mod_2nd, bootstrap = FALSE,
                             mod_whole = mod_bo_psea[[1]]) {
  # presumably must use same bootstrap for all 3
  if (bootstrap) {
    mf_pairs_tmp <- mf_pairs_reg[sample(nrow(mf_pairs_reg, replace = TRUE)),]
    mod_1st <- update(mod_1st, data = mf_pairs_tmp)
    mod_2nd <- update(mod_2nd, data = mf_pairs_tmp)
    mod_whole <- update(mod_whole, data = mf_pairs_tmp)
  }
  
  mediator_effect_bo <- tidy(mod_1st) %>% 
                          filter(term == "birth_order.x") %>%
                          pull(estimate)
  psea_effect_mediator <- tidy(mod_2nd) %>% 
                          filter(term == mediator) %>% 
                          pull(estimate)
  psea_effect_bo <- tidy(mod_bo_psea[[1]]) %>% 
                     filter(term == "birth_order.x") %>% 
                     pull(estimate)
  
  mediator_effect_bo * psea_effect_mediator / psea_effect_bo
}


mod2_meds <- c(
  University = mediation_pct("university.xTRUE", mod_bo_1st[["University"]], 
                               mod_bo_psea[[2]]),
  Height     = mediation_pct("height.x", mod_bo_1st[["Height"]],
                               mod_bo_psea[[2]]),
  `Fluid IQ` = mediation_pct("fluid_iq.x", mod_bo_1st[["Fluid IQ"]],
                               mod_bo_psea[[2]])
)

mod3_meds <- c(
  Income     = mediation_pct("first_job_pay.x", mod_bo_1st[["Income"]], 
                               mod_bo_psea[[3]]),
  Height     = mediation_pct("height.x", mod_bo_1st[["Height"]],
                               mod_bo_psea[[3]]),
  `Fluid IQ` = mediation_pct("fluid_iq.x", mod_bo_1st[["Fluid IQ"]],
...
# we do this to avoid a collinear birth_order.x == 6, n_sibs.x == 6
# pair of dummies
fml_bo_psea_dummies <- fml_bo_psea %>% 
                        purrr::map(update_with_birth_order_dummies) %>% 
                        purrr::map(update, . ~ . - factor(birth_mon.x), rhs = 2)

mod_bo_psea_dummies <- purrr::map(fml_bo_psea_dummies, fixest::feols, 
                         data = mf_pairs_reg,
                         notes = FALSE
                       )
...
wald_tests <- purrr::map(mod_bo_psea_dummies, fixest::wald, 
                           keep    = "birth_order",
                           se      = tidy_args$se,
                           print   = FALSE
                         )
...
huxtable::huxreg(mod_bo_psea_dummies, 
       ...
       insert_row(after = 24, "Family size dummies", rep("Yes", 4)) %>% 
       insert_row(after = 25, "Birth month dummies", rep("Yes", 4)) %>% 
       insert_row(after = 26, "Birth year dummies", rep("Yes", 4)) %>% 
       insert_row(after = 27, "Controls (IQ, height)", 
                    c("No", rep("Yes", 3))) %>% 
       set_bottom_border(25:27, everywhere, 0) %>% 
...
huxreg_coefs <- grep("factor\\(birth_order", names(coef(mod_bo_males_dummies[[1]])),
                       value = TRUE)
names(huxreg_coefs) <- gsub(".*(\\d).*$", "Birth order \\1",
                            huxreg_coefs)
huxreg_coefs <- c(
...
huxtable::huxreg(reg_list, 
       ...
       insert_row(after = 22, "Family size dummies", rep("Yes", 6))   %>% 
       insert_row(after = 23, "Birth month dummies", rep("Yes", 6))   %>% 
       insert_row(after = 24, "Birth year dummies", rep("Yes", 6))    %>% 
       insert_row(after = 25, "Controls (IQ, height)", rep("Yes", 6)) %>% 
