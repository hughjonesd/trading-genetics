...
[@breen2011educational; @greenwood2014marry] and contribute to its persistence
across generations, which is surprisingly high
[@clark2015intergenerational;@solon2018we]. Wealthy families pass on advantages
to their children through both genetic inheritance and environmental influence
[@Rimfeld_2018; @bjorklund2006origins; @sacerdote2011nature].
...
[^interaction]: Although @muslimova2020dynamic find that PSEA and birth order
...
transmissible socioeconomic status (e.g. wealth). @heath1985resolving, following earlier papers [@rao1976resolution; @rao1979path], introduce
...
investments in children’s human capital [@cunha2007technology;
@cunha2010estimating; @heckman2014economics; @kong2018nature], or if high-income
parents can invest more in transmitting their human capital [@becker2018theory]. 
Thus, by bringing "good genes"[^language] and enriched environments together, 
...
[^prev-work]: As well as the work mentioned above on cultural and 
...
effect, we can reject $a = 1$.[^if-a-zero]

[^if-a-zero]: Conceivably, if $a = 0$ but there is a pre-existing correlation 
...
regressions based on equation \@ref(eq:mediator-model) for each
...
`r pretty(-coef_bo["Income"])` standard deviations, fluid IQ by about 
...
can separate the effect of parental age from birth order.[^greg-its-fine] This
...
[^greg-its-fine]: A possible critique is that later siblings by definition 
...
relationships already.[^age-25] Data on IQ and self-reported health is 
...
and females.[^no-kids] As in Britain, coefficients look very
...
we run @oster2019unobservable style robustness checks where we formally ask
...
As noted earlier, our results could conceivably be explained by spouses *only*
...
suggests that own PSEA affects spouse's education [@robinson2017genetic;
@torvik2022modeling]. To confirm this we use a sample of siblings from the MoBa
data[^why-not-ukbb], and regress spouse's university attendance on own PSEA,
...
[^why-not-ukbb]: The UK Biobank sample has too few siblings with spouses for
this analysis to be informative.
...
generations [@clark2015intergenerational; @clark2023inheritance]. One problem
...
[^quotes]: The appendix has a selection of relevant historical quotations.
...
Lastly, we run an @oster2019unobservable style robustness analysis on the
...
In UK Biobank, we calculate the maximum $R^2$ as `r R2max`.[^comparison] For university
attendance, $\delta^*$ is `r delta_uni`, and for income in first job 
$\delta^*$ is `r delta_fjp`. In MoBa, we calculate the maximum $R^2$ as 
`r R2max_moba`.[^why-bigger] For university attendance, $\delta^*$ is `r delta_uni_moba`, 
and for income at age 30, $\delta^*$ is `r delta_inc_moba`. These results 
...
[^comparison]: In other regressions of spouse traits on
PSEA, $R^2$ are indeed below this value [@hugh2016assortative; @okbay2022polygenic].

[^why-bigger]: The MoBa number is larger because the $R^2$ of EA4 on education is
higher than for EA3.
...
Abilities come from innate talents, which differ in their capacities
and take on different responsibilities in government.
