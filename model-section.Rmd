...
eqm_correlation <- function (a, tau, theta, gamma) {
  eqm_cov <- eqm_covariance(a, tau, theta, gamma)
  eqm_cov["sigma"] / sqrt(eqm_cov["s2"] * eqm_cov["S2"])
}
...
eqm_sd_x1 <- function (a, tau, theta, gamma) {
  eqm_cov <- eqm_covariance(a, tau, theta, gamma)
  sqrt(eqm_cov["s2"])
}
```
...
```{r pic-asymptotic-inequality, fig.align = "center", fig.cap = "Meritocracy, assortative mating and economic inequality. Curves show equilibrium standard deviation of SES $x_2$ for different parameter combinations, by importance of genetics in spouse matching ($a$). Solid line: persistent wealth ($\\theta$ = 0.9, $\\gamma$ = 0). Dashed line: meritocracy ($\\theta$ = 0.3, $\\gamma$ = 0.3).", fig.height = 3.5, fig.width = 3.5}

# These curves show that in the presence of assortative mating, 
# a small amount of meritocracy induces a large G-E correlation:
# curve(Vectorize(eqm_correlation)(a = 0.5, tau = 0.95, theta = x, gamma = 0),
#       xlim = c(0, 0.9), ylim = c(0, 1), 
#       ylab = "Corr(x1, x2)", xlab = "θ", cex.axis = 0.8, las = 1)
# curve(Vectorize(eqm_correlation)(a = 0.5, tau = 0.95, theta = x, gamma = 0.2),
#       add = TRUE, lty = 2)
# legend("topleft", legend = c("θ = 0", "θ = 0.2"), lty = 1:2, cex = 0.6)
# curve(Vectorize(eqm_correlation)(a = 0.5, tau = 0.95, theta = 0, gamma = x),
#       xlim = c(0, 0.9), ylim = c(0, 1), 
#       ylab = "Corr(x1, x2)", xlab = "γ", cex.axis = 0.8, las = 1)
# curve(Vectorize(eqm_correlation)(a = 0.5, tau = 0.95, theta = 0.2, gamma = x),
#       add = TRUE, lty = 2)
# legend("topleft", legend = c("γ = 0", "γ = 0.2"), lty = 1:2, cex = 0.6)

# These curves show that a small amount of meritocracy induces a large 
# G-E correlation even compared to assortative mating, and even when 
# persistence of wealth is very high
# curve(Vectorize(eqm_correlation)(a = x, tau = 0.95, theta = 0.9, gamma = 0),
#       xlim = c(0, 1), ylim = c(0, 1), 
#       ylab = "Corr(x₁, x₂)", xlab = "a", cex.axis = 0.8, las = 1)
# curve(Vectorize(eqm_correlation)(a = x, tau = 0.95, theta = 0.2, gamma = 0),
#       add = TRUE, lty = 2)
# curve(Vectorize(eqm_correlation)(a = x, tau = 0.95, theta = 0.2, gamma = 0.2),
#       add = TRUE, lty = 3)
# legend("topleft", 
#        legend = c("θ = 0.9, γ = 0", "θ = 0.2, γ = 0", "θ = 0.2, γ = 0.2"), 
#        lty = 1:3, cex = 0.6)


# These graphs show:
# (1) If you reduce persistence of income (second line), inequality decreases
# (2) But if you simultaneously increase meritocratic mobility,
#     inequality need not decrease.
# (3) In particular, meritocracy interacts with the importance of genetics
#     in matching markets to increase inequality.
old_par <- par(mar = c(4, 4, 0.1, 0.1))
curve(Vectorize(eqm_sd_x2)(a = x, tau = 0.95, theta = 0.9, gamma = 0),
      xlim = c(0, 1), ylim = c(1, 3), 
      ylab = expression(sd(x[2])), xlab = "a", cex.axis = 0.8, las = 1, lwd = 2)
# curve(Vectorize(eqm_sd_x2)(a = x, tau = 0.95, theta = 0.3, gamma = 0),
#       add = TRUE, lty = 2, col = "red2", lwd = 2)
curve(Vectorize(eqm_sd_x2)(a = x, tau = 0.95, theta = 0.3, gamma = 0.3),
      add = TRUE, lty = 2, col = "steelblue", lwd = 2)

legend("topleft", 
       legend = expression(list(theta == 0.9, gamma == 0), 
                           list(theta == 0.3, gamma == 0.3)), 
       lty = 1:2, col = c("black", "steelblue"),
       y.intersp = 0.85, cex = 0.85, lwd = 2)

par(old_par)

# f <- function (x, y) eqm_sd_x2(a = x, tau = 0.95, theta = 0.3, gamma = y)
# rgl::plot3d(Vectorize(f),
#              xlim  = c(0, 1),
#              ylim  = c(0, 0.8), # 1 gives an error
#              zlab  = "",
#              xlab  = "a", 
#              ylab = "γ",
#              col   = "red",
