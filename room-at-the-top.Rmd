...
without mediators. Confidence intervals are estimated by 199 bootstraps.
...
compute_mediation_effects <- function (resample = FALSE) {
  ...
  psea_effect_bo <- coef(mod_bo_psea_boot[[1]])["birth_order.x"]
  
  effects_uni <- mediator_effect_bo["University"] * 
    psea_effect_mediators[, "university.xTRUE"] / psea_effect_bo
  effects_income <- mediator_effect_bo["Income"] * 
    psea_effect_mediators[, "first_job_pay.x"] / psea_effect_bo
  effects_height <- mediator_effect_bo["Height"] * 
    psea_effect_mediators[, "height.x"] / psea_effect_bo
  effects_fluid_iq <- mediator_effect_bo["Fluid IQ"] * 
    psea_effect_mediators[, "fluid_iq.x"] / psea_effect_bo
...
# for each 1st stage model, run 100 bootstraps and gather coefs
n_reps <- 199
bootstraps <- replicate(n_reps, compute_mediation_effects(resample = TRUE))
# bootstraps is now 3 x 4 x n_reps
cis <- apply(bootstraps, 1:2, quantile, c(0.025, 0.975), na.rm = TRUE)
...
as_huxtable(med_matrix, add_colnames = TRUE, add_rownames = "") %>% 
      ...
      add_footnote("Bootstrap 95% confidence intervals in brackets") 
