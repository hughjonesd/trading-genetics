...
estimate the effect of socio-economic status (as an effect of the random
assignment of birth order) on spouse PSEA.

The reduced-form regression of spouse PSEA on birth order (henceforth
BO) is:
...
where $\textbf{X}$ is a vector of predetermined controls (own year of
birth, own PSEA, father or mother's year of birth).
...
-   TODO: Find adequate terminology for the set of proximate
    outcomes/investments. Maybe gains?
...
and $J_m$ and $J_u$ are the index sets of mediators of attractiveness
which are measured and unmeasured.
...
mod_bo_1st$income <- lm(first_job_pay ~ n_older_sibs + factor(n_sibs), famhist, 
                    subset = n_sibs >= 2 & n_sibs <= 8)
...
famhist %>% 
      ...
      mutate(
        `Birth order` = factor(n_older_sibs + 1),
        `Family size` = n_sibs,
...
fml_bo_psea[[3]] <- EA3.y ~ first_job_pay.x + n_older_sibs.x | factor(n_sibs.x)
fml_bo_psea[[4]] <- EA3.y ~ first_job_pay.x + university.x + n_older_sibs.x |
      factor(n_sibs.x)
...
huxreg(mod_bo_psea, 
       ...
       insert_row(after = 13, "Birth year dummies", rep("Yes", 4)) %>% 
...
birth order of male respondents to predict female spouses' PSEA. Columns
3 and 4 use female respondents to predict male spouses' PSEA. Effect
sizes are larger for female respondents. This might be because social
status matters more for females in the marriage markets (contrary to a
common stereotype), or because males place less value on phenotypes
related to PSEA. For both sexes, controlling for university attendance
reduces the effect of birth order. Columns 5 and 6 use only couples with
children. The effect of birth order is slightly larger than for the
sample as a whole, and university attendance still seems to mediate the
birth order effect.
...
huxreg(reg_list,
       ...
       insert_row(after = 9, "Family size dummies", rep("Yes", 6)) %>% 
       insert_row(after = 10, "Birth month dummies", rep("Yes", 6)) %>% 
       insert_row(after = 11, "Birth year dummies", rep("Yes", 6)) %>% 
...
check_cor <- function (score_name) {
  ...
  mod_check <- lm(fml_check, famhist, n_sibs >= 2 & n_sibs <= 8 & ! is.na(n_older_sibs))
  mod_check %>% 
        tidy() %>% 
...
check_cor_within <- function (score_name) {
  fml_check_within <- as.formula(paste(score_name, 
                        "~ 0 + factor(n_sibs) + factor(n_sibs):factor(n_older_sibs)"))
  mod_check <- lm(fml_check_within, famhist, 
                  n_sibs >= 2 & n_sibs <= 8 & ! is.na(n_older_sibs))
  # NB: the model creates NA estimates for when n_sibs <= n_older_sibs
  # since these have 0 rows in the data. I think this is not a problem.
  mod_check %>% 
        tidy() %>% 
        filter(grepl("n_older_sibs", term)) %>% 
...
To check this, we run balance tests on 33 different polygenic
...
huxreg(mod_bo_no_fath_age, 
       ...
       insert_row(after = 11, "Birth year dummies", rep("Yes", 4)) %>% 
       set_bottom_border(10:11, everywhere, 0) %>% 
...
huxtable::huxreg(mod_bo_psea_ix, 
       ...
       insert_row(after = 23, "Birth year dummies", rep("Yes", 4)) %>% 
