...
complementary [@becker1973theory]. Other, non-individualistic interpretations 
...
Y_b = \kappa_b + \sum_{j \in J_m} \alpha_b^j \theta^j_b+\sum_{j \in J_u} \alpha_b^j \theta^j_b + \mathbf{X'} \mathbf{\beta}_b + \tilde{\varepsilon}_b = \tau_b + \sum_{j \in J_m} \alpha_b^j \theta^j_b + \mathbf{X'} \mathbf{\beta}_b + \varepsilon_b 
\end{equation}
...
coefficients on the interaction terms ($\mathbf{\alpha}_b=0$ and
$\mathbf{\beta_b}=0$). See @heckman2013understanding and @fagereng2021wealthy 
for details and different applications.

We use a linear model for each observed mediator variable:
\begin{equation}
\label{eq:mediator-model}
\theta^j_b=\mu_{0,j}+\textbf{X'}\mathbf{\mu_{1,j}}+\mu_{2,j}\cdot b+\eta_j, j \in J_m  
\end{equation}
...
Y_b = \tau_0+\tau b + \sum_{j \in J_m} \alpha^j (\mu_{0,j}+\textbf{X'}\mathbf{\mu_{1,j}}+\mu_{2,j}\cdot b+\eta_j) + \mathbf{X'} \mathbf{\beta} + \varepsilon_b 
\end{equation}
...
Y = \tau_0 + \tau B + \sum_{j \in J_m} \alpha^j \theta^j_b + \mathbf{X'} \mathbf{\beta} + \varepsilon
\end{equation}
